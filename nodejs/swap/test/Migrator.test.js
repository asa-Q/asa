...
const EZZCPair = artifacts.require('EZZCPair');
const EZZCFactory = artifacts.require('EZZCFactory');
const Migrator = artifacts.require('Migrator');

contract('Migrator', ([alice, bob, dev, minter]) => {
    beforeEach(async () => {
        this.factory1 = await EZZCFactory.new(alice, { from: alice });
        this.factory2 = await EZZCFactory.new(alice, { from: alice });
        ...
        this.lp1 = await EZZCPair.at((await this.factory1.createPair(this.weth.address, this.token.address)).logs[0].args.pair);
        this.lp2 = await EZZCPair.at((await this.factory2.createPair(this.weth.address, this.token.address)).logs[0].args.pair);
    ...
    it('should allow first minting from public only after migrator is gone', async () => {
        ...
        this.lpx = await EZZCPair.at((await this.factory2.createPair(this.weth.address, this.tokenx.address)).logs[0].args.pair);
